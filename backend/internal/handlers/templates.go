...
func (h *TemplateHandler) List(c *fiber.Ctx) error {
	...
	rows, err := h.DB.Query(ctx, `
		SELECT id, name, width_mm, height_mm, file_type, qr_x_ratio, qr_y_ratio, qr_size_ratio, created_at
	...
	for rows.Next() {
		var r templateRow
		if err := rows.Scan(&r.ID, &r.Name, &r.WidthMM, &r.HeightMM, &r.FileType,
			&r.QRXRatio, &r.QRYRatio, &r.QRSizeRatio, &r.CreatedAt); err == nil {
			out = append(out, r)
		}
	}
	return c.JSON(out)
...
func (h *TemplateHandler) Get(c *fiber.Ctx) error {
	...
	err = h.DB.QueryRow(ctx, `
		...
		&r.ID, &r.Name, &r.WidthMM, &r.HeightMM, &r.FileType,
		&r.QRXRatio, &r.QRYRatio, &r.QRSizeRatio, &r.CreatedAt)
	if err != nil {
...
func (h *TemplateHandler) Create(c *fiber.Ctx) error {
	...
	var id int64
	err = h.DB.QueryRow(ctx, `
		INSERT INTO label_templates (name, width_mm, height_mm, file_type, file_path, created_by)
		VALUES ($1,$2,$3,$4,$5,$6)
		RETURNING id
	`, name, widthMM, heightMM, fileType, fullPath, nullIfZeroInt64(adminID)).Scan(&id)
	if err != nil {
...
func (h *TemplateHandler) Update(c *fiber.Ctx) error {
	...
	if b.QRXRatio != nil && (*b.QRXRatio < 0 || *b.QRXRatio > 1) {
		return c.Status(400).JSON(fiber.Map{"error": "qr_x_ratio_out_of_range"})
	}
	if b.QRYRatio != nil && (*b.QRYRatio < 0 || *b.QRYRatio > 1) {
		return c.Status(400).JSON(fiber.Map{"error": "qr_y_ratio_out_of_range"})
	}
	if b.QRSizeRatio != nil && (*b.QRSizeRatio <= 0 || *b.QRSizeRatio > 1) {
	...
	tag, err := h.DB.Exec(ctx, `
		...
		WHERE id = $5
	`, b.Name, b.QRXRatio, b.QRYRatio, b.QRSizeRatio, id)
	if err != nil {
