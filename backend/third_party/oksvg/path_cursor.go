...
func (c *PathCursor) AddArcFromA(points []float64) {
	...
	// Use the local (correctly-offset) points slice, not c.points, so
	// chained arc groups within a single A/a command (e.g. "A2,2,0,0,1,x1,y1
	// 2,2,0,0,1,x2,y2 ...") each use their own radius/rotation/endpoint
	// instead of always reusing the first group's values.
